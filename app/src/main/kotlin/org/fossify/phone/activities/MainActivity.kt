...
class MainActivity : SimpleActivity() {
    ...
    override fun onBackPressed() {
        if (isFinishing) { // Add this check before accessing binding
            super.onBackPressed()
            return
        }
        if (binding.mainMenu.isSearchOpen) {
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mainMenu.apply {
            ...
            onSearchClosedListener = {
                if (this@MainActivity.isFinishing) { // Corrected check
            ...
            getToolbar().setOnMenuItemClickListener { menuItem ->
                if (isFinishing) return@setOnMenuItemClickListener true // Add this check
    ...
    private fun checkContactPermissions() {
        handlePermission(PERMISSION_READ_CONTACTS) {
            if (isFinishing) return@handlePermission // Add this check
            initFragments()
        }
    }
    ...
    private fun initFragments() {
        ...
        binding.mainTabsHolder.onGlobalLayout {
            if (isFinishing) return@onGlobalLayout // Add this check
            Handler().postDelayed({
                if (isFinishing) return@postDelayed // Add this check
    ...
    private fun setupTabs() {
        ...
        binding.mainTabsHolder.onTabSelectionChanged(
            tabUnselectedAction = {
                if (isFinishing) return@onTabSelectionChanged // Add this check
    ...
    private fun getContactsFragment(): ContactsFragment? = if (!isFinishing) findViewById(R.id.contacts_fragment) else null
    private fun getFavoritesFragment(): FavoritesFragment? = if (!isFinishing) findViewById(R.id.favorites_fragment) else null
    private fun getRecentsFragment(): RecentsFragment? = if (!isFinishing) findViewById(R.id.recents_fragment) else null
    ...
    private fun showSortingDialog(showCustomSorting: Boolean) {
        ChangeSortingDialog(this, showCustomSorting) {
            if (isFinishing) return@ChangeSortingDialog // Add this check
            getFavoritesFragment()?.refreshItems {
                if (isFinishing) return@refreshItems // Add this check
                if (binding.mainMenu.isSearchOpen) {
    ...
    private fun showFilterDialog() {
        FilterContactSourcesDialog(this) {
            if (isFinishing) return@FilterContactSourcesDialog // Add this check
            getFavoritesFragment()?.refreshItems {
                if (isFinishing) return@refreshItems // Add this check
                if (binding.mainMenu.isSearchOpen) {
                    getCurrentFragment()?.onSearchQueryChanged(binding.mainMenu.getCurrentQuery())

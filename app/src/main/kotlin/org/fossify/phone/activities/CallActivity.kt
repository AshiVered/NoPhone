...
class CallActivity : SimpleActivity() {
    ...
    private fun handleSwipe() = binding.apply {
        ...
        callDraggable.setOnTouchListener { _, event ->
            if (isFinishing) return@setOnTouchListener true // Add this check
            when (event.action) {
    ...
    private fun findVisibleViewsUnderDialpad(): Sequence<Pair<View, Float>> {
        if (isFinishing) return emptySequence() // Add this check
            ...
            callerAvatar.apply {
                if (avatarUri.isNullOrEmpty()) {
                    ...
                    if (!isFinishing && !isDestroyed) {
                        Glide.with(this@CallActivity) // Use this@CallActivity to ensure correct context
                            .load(avatarUri)
    ...
    private fun updateCallContactInfo(call: Call?) {
        getCallContact(applicationContext, call) { contact ->
            if (isFinishing) return@getCallContact // Add this check
